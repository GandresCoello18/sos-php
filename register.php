...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["username"]))){
        ...
        if($stmt = $pdo->prepare($sql)){
            ...
            if($stmt->execute()){
                if($stmt->rowCount() == 1){
                    $username_err = "El nombre de usuario ya está usado.";
                } else{
                    $username = trim($_POST["username"]);
                }
            } else{
                echo "Oops! Algo salio mal. Por favor intente más tarde.";
            }
        }
    ...
    if(empty(trim($_POST["password"]))){
        ...
        $password_err = "Password debe tener al menos 6 caracteres.";
    } else{
    ...
    if(empty(trim($_POST["confirm_password"]))){
        ...
        if(empty($password_err) && ($password != $confirm_password)){
            $confirm_password_err = "Password no coinciden.";
        }
    }
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err)){
        ...
        if($stmt = $pdo->prepare($sql)){
            ...
            if($stmt->execute()){
                ...
                echo "Algo salio mal. Por favor intente más tarde.";
            }
        }
